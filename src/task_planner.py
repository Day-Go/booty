...
class TaskPlanner:
    ...
    def analyze_request(self, user_message: str) -> Dict[str, Any]:
        ...
        file_operations = 0
        file_operations += len(re.findall(r"(?:read|write|check|view|open|update).*file", user_message, re.IGNORECASE))
        file_operations += len(re.findall(r"content(?:s)? of.*\.[a-z]+", user_message, re.IGNORECASE))
        
        # Look for search operations
        search_operations = 0
        search_operations += len(re.findall(r"(?:search|find|look for|locate|grep)", user_message, re.IGNORECASE))
        ...
        complex_indicators = [
            "refactor", "restructure", "implement", "create a new", "analyze", 
            "compare", "optimize", "fix", "debug", "add feature", "multiple files"
        ]
        
        for indicator in complex_indicators:
            if re.search(r'\b' + re.escape(indicator) + r'\b', user_message, re.IGNORECASE):
                result["complexity"] = "complex"
                result["estimated_resource_usage"] = "high"
                result["requires_planning"] = True
                break
    ...
    def _generate_potential_subtasks(self, 
                                    user_message: str, 
                                    file_operations: int,
                                    search_operations: int) -> List[str]:
        ...
        if search_operations > 0:
            subtasks.append("Information gathering: Search for relevant files and code sections")
        ...
        file_extensions = re.findall(r'\b\w+\.(py|js|java|cpp|h|json|yaml|yml|md|txt)\b', user_message)
        if file_extensions:
            unique_extensions = set(file_extensions)
            file_types = ", ".join(unique_extensions)
            subtasks.append(f"File processing: Handle {file_types} files mentioned in request")
        
        # Look for potential implementation tasks
        if re.search(r'\b(implement|create|add|develop)\b', user_message, re.IGNORECASE):
            subtasks.append("Code implementation: Write or modify code based on requirements")
