...
class TestStreamingXMLParserWithFixtures:
    ...
    def test_chunked_command_detection(self, sample_mcp_commands):
        ...
        for cmd_type, command in sample_mcp_commands.items():
            ...
            assert detected_command == command, (
                f"Failed to detect {cmd_type} command in chunks"
            )
    ...
    def test_command_with_think_blocks(self, sample_mcp_commands):
        ...
        for cmd_type, command in sample_mcp_commands.items():
            ...
            detected_command = self.parser.get_command()
            assert detected_command == command, (
                f"Failed to detect {cmd_type} command after think block"
            )
    ...
    def test_interleaved_commands_and_think_blocks(self, sample_mcp_commands):
        ...
        # Check that the first command was detected
        ...
        # Now manually check for additional commands in the buffer
        buffer_has_cmd = self.parser.check_for_mcp_commands()
        assert buffer_has_cmd is True, "Second command not found in buffer"
    ...
    def test_malformed_command_recovery(self, sample_mcp_commands):
        ...
        detected_command = self.parser.get_command()
        assert valid_cmd in detected_command, (
            "Failed to recover and detect valid command after malformed input"
        )
