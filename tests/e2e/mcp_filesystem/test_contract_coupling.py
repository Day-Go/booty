...
def test_mock_command_handler_interface():
    ...
    real_methods = {name for name, obj in inspect.getmembers(MCPCommandHandler) 
                   if not name.startswith('_') and callable(obj)}
    
    # Get public methods from mock command handler
    mock_methods = {name for name, obj in inspect.getmembers(MockMCPCommandHandler) 
                   if not name.startswith('_') and callable(obj)}
    
    # Essential methods that must be implemented
    essential_methods = {'extract_file_commands', 'execute_file_commands', 
    ...
    other_methods = real_methods - essential_methods - {'__init__', '__dict__', '__weakref__'}
    other_missing = other_methods - mock_methods
    if other_missing:
        print(f"Note: Mock could implement these non-essential methods: {other_missing}")
...
def test_mock_parser_interface():
    ...
    real_methods = {name for name, obj in inspect.getmembers(StreamingXMLParser) 
                   if not name.startswith('_') and callable(obj)}
    
    # Get public methods from mock parser
    mock_methods = {name for name, obj in inspect.getmembers(MockStreamingXMLParser) 
                   if not name.startswith('_') and callable(obj)}
...
def test_command_extraction_parity():
    ...
    assert len(real_result) == len(mock_result), "Different number of commands extracted"
    
    if real_result and mock_result:
        assert real_result[0]["action"] == mock_result[0]["action"], "Different actions extracted"
        assert real_result[0]["path"] == mock_result[0]["path"], "Different paths extracted"
...
def test_basic_filesystem_command_formats():
    ...
    for command_xml, expected in test_cases:
        ...
        # Check action
        assert cmd["action"] == expected["action"], f"Action mismatch: got {cmd['action']}, expected {expected['action']}"
        
        # Check path if present
        if "path" in expected:
            assert cmd["path"] == expected["path"], f"Path mismatch: got {cmd.get('path')}, expected {expected['path']}"
