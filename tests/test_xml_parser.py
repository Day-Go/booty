...
class TestStreamingXMLParser:
    ...
    def test_think_block_in_single_token(self):
        ...
        self.parser.feed("<think>ignore this</think><mcp:filesystem><pwd /></mcp:filesystem>")
    ...
    def test_partial_command_detection(self):
        ...
        assert len(self.parser.xml_stack) == 2  # Should have mcp:filesystem and read tags
        ...
        command = self.parser.get_command()
        assert "<mcp:filesystem><read path='/some/file.txt' /></mcp:filesystem>" == command
    ...
    def test_extract_complete_xml_fallback(self):
        ...
        assert commands[0] == "<mcp:filesystem><read path='/file.txt' /></mcp:filesystem>"
